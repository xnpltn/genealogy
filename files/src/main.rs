...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    app.global::<TableData>().on_show_update_window({
        ...
        move || {
            ...
            update_window.global::<TableData>().on_close_request({
        ...
        move || {
            let _ = slint::spawn_local({
                ...
                async move {
                    ...
                    .unwrap();
                    create_window.global::<TableData>().on_close_request({
    ...
    app.global::<TableData>().on_current_row_changed({
        ...
        move |row| {
            ...
            //    .global::<TableData>()
            //    .set_active_note(Not);
            let mut table_data = weak_app.unwrap().global::<TableData>().get_relative();
            if active_tab == 1 {
                table_data = weak_app.unwrap().global::<TableData>().get_relative();
            } else if active_tab == 2 {
                table_data = weak_app.unwrap().global::<TableData>().get_females();
            } else if active_tab == 3 {
                table_data = weak_app.unwrap().global::<TableData>().get_employees();
            }
            let table_data = table_data.row_data(row as usize).unwrap();
        ...
        move |row, id| {
            ...
            if id == "" {
                ...
                if active_tab == 1 {
                    ...
                    let table_data = weak_app.unwrap().global::<TableData>().get_females();
                    if let Some(table_data) = table_data.row_data(row as usize) {
                        if let Some(id) = table_data.row_data(0) {
                            let id = id.text;
                            let path = FileDialog::new()
                    ...
                    let table_data = weak_app.unwrap().global::<TableData>().get_employees();
                    if let Some(table_data) = table_data.row_data(row as usize) {
                        if let Some(id) = table_data.row_data(0) {
                            let id = id.text;
                            let path = FileDialog::new()
...
async fn get_relative_data(
    ...
    let deathday: String = row.get("died_at");
    ...
    let mut death_date = String::new();
    if deathday.len() > 7{
        death_date = utils::from_sqlite_to_date(deathday);
    }
...
fn update_relative(
    ...
    let _ = slint::spawn_local({
        async move {
            ...
            if relative.birthday.to_string().len() > 0{

            if !utils::is_valid_date(&relative.birthday.to_string()) {
                update_weak_window
                    .unwrap()
                    .global::<TableData>()
                    .set_update_eror(SharedString::from("Invalid Date (Birthday)"));
                return;
            }
            }
            if relative.deathday.to_string().len() > 0{
                if !utils::is_valid_date(&relative.deathday.to_string()) {
                    update_weak_window
                        .unwrap()
                        .global::<TableData>()
                        .set_update_eror(SharedString::from("Invalid Date (Died)"));
                    return;
            }

            }
            let mut sqlite_date = String::new();
            let mut sqlite_death_date= String::new();
            if !utils::is_valid_state(&relative.state.to_string()){
                    update_weak_window
                        .unwrap()
                        .global::<TableData>()
                        .set_update_eror(SharedString::from("Invalid State"));
                    return;
            ...
            if relative.deathday.to_string().len() > 0{
            match utils::sqlite_date(relative.deathday.to_string()) {
                Ok(date) => {
                    sqlite_death_date = date;
                }
                Err(_) => {
                    update_weak_window
                        .unwrap()
            ...
            if mother_id_db > 0 && father_id_db > 0 {
                ...
                let res = sqlx::query(&sql::update_both_parents())
                    ...
                    .bind(relative.crazy)
                    .bind(sqlite_death_date.clone())
                    .bind(relative.address.to_string())
                    .bind(relative.state.to_string())
                    .bind(id.to_string())
                    .execute(&pool)
                    .await;
                ...
                match res {
                    Ok(_) => {
                        ...
                        let _ = slint::spawn_local({
                            let pool = pool.clone();
                            async move {
                                let _ = update_global_data(pool.clone(), Rc::clone(&app)).await;
                                if let Ok(relative) = get_relative_data(&relative.id.to_string(), &pool).await{

                                    Rc::clone(&app).unwrap().global::<TableData>().set_active_relative(relative);
                                }
                            }
                        });
                ...
                match res {
                    Ok(_) => {
                        ...
                        let _ = slint::spawn_local({
                            ...
                            async move {
                                let _ = update_global_data(pool.clone(), Rc::clone(&app)).await;
                                if let Ok(relative) = get_relative_data(&id.clone(), &pool).await{
                                    Rc::clone(&app).unwrap().global::<TableData>().set_active_relative(relative);
        ...
        async move {
            ...
            if !utils::is_valid_date(&relative.birthday.to_string()) {
                weak_create_window
                    .unwrap()
                    .global::<TableData>()
                    .set_create_error(SharedString::from("Invalid Date Format (Birthday)"));
                return;
            }
            ...
            if !utils::is_valid_date(&relative.deathday.to_string()){
                weak_create_window
                    .unwrap()
                    .global::<TableData>()
                    .set_create_error(SharedString::from("Invalid Date Format (Died)"));
                return;
            }
            }
            let mut sqlite_date = String::new();
            ...
            match utils::sqlite_date(relative.birthday.to_string()) {
                ...
                Err(_) => {
                    weak_create_window
                        .unwrap()
                        .global::<TableData>()
                        .set_create_error(SharedString::from("Invalid Date Forma (Died)"));
                    drop(sqlite_date);
                    return;
                }
            }
            ...
            if !utils::is_valid_state(&relative.state.to_string()){
                    weak_create_window
                        .unwrap()
                        .global::<TableData>()
                        .set_create_error(SharedString::from("Invalid State"));
                    drop(sqlite_death_date);
                    return;
                ...
                match res {
                    Ok(_row) => {
                        ...
                        if let Some(fname) = file_path.file_name() {
                            if let Some(_name) = fname.to_str() {
                            }
                        }
